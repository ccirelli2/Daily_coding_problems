...
def get_combinations(ran_weights):
    ...
    for _ in range(0, len(ran_weights)):
...
def get_optimal_combo_boat_n(list_all_combos):
    ...
    for combo in list_all_combos:
        ...
        # If the count is > 0, then we have a value appended to the list to
        # compare.
            ...
            # If the Length of the Current Combo is > then prior, remove prior
            # and append new.
            ...
            # If the lengths are equal, check which has the lower sum
                ...
                # If the sum of the new one is >, then replace prior with new
                # combo.
                if sum(combo) > prior_combo_sum:
                    combo_list.remove(combo_list[0])
                    combo_list.append((len(combo), sum(combo), combo))
                    Count += 1
                # If the sum is equal or less, then lets do nothing as they are
                # equivalent.
                else:
                    Count += 1
